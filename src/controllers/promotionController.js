...
const constructCommissionQuery = (startDate, endDate, phone, limit, offset) => {
  const queries = {
    Commissions: {
      query: `SELECT 'Commission' AS name, time,money,from_user_phone FROM commissions WHERE phone = ? AND time >= ? AND time <=?`,
      count: `SELECT COUNT(*) AS totalCount FROM (SELECT time FROM commissions WHERE phone = ? AND time >= ? AND time <=? ) AS grouped`,
    },
  };
...
const listCommission = async (req, res) => {
  ...
  const { transactionsQuery, totalCountQuery, params } =
    constructCommissionQuery(startDate, endDate, userInfo.phone, limit, offset);
...
const subordinatesDataByTimeAPI = async (req, res) => {
  try {
    ...
    // return res.status(401).json({ message: startDate });
