...
const totalJoin = async (req, res) => {
  ...
  if (rows.length > 0) {
    const [wingoall] = await connection.query(
      `SELECT * FROM minutes_1 WHERE game = "${game}" AND status = 0 ORDER BY id ASC `,
      [auth],
    );
...
const listCommission = async (req, res) => {
  ...
  if (search) {
    sql += " WHERE (phone LIKE ? OR id LIKE ?)";
    countSql += " AND (phone LIKE ? OR phone LIKE ?)";
    params = [`%${search}%`, `%${search}%`];
  }
...
const listRedenvelops = async (req, res) => {
  ...
  let [redenvelopes] = await connection.query(
    "SELECT r.*, COUNT(ru.id_redenvelops) AS used_count FROM redenvelopes r LEFT JOIN redenvelopes_used ru ON r.id_redenvelope = ru.id_redenvelops GROUP BY r.id_redenvelope ORDER BY r.time DESC",
  );
...
const fetchMemberDetails = async (req, res) => {
  const { userPhone, userType } = req.body;

  try {
    const [rows] = await connection.query(
      `SELECT * FROM user_bank WHERE phone = ? AND tp = ? ORDER BY time DESC LIMIT 1`,
      [userPhone, userType],
    );
    ...
    const userData = rows[0]; // take the latest matched record

    return res.status(200).json({
      message: "Success",
      status: true,
      data: {
        bank_name: userData.name_bank || "",
        name: userData.name_user || "",
        stk: userData.stk || "",
        ifsc: userData.chi_nhanh || "",
      },
    });
  } catch (error) {
...
const updateMemberDetails = async (req, res) => {
  try {
    ...
    if (!userPhone || !userType) {
      return res.status(400).json({
        status: false,
        message: "Missing required fields: userPhone or userType.",
      });
    }
    ...
    if (userType === "BANK_CARD") {
      const userBankName = req.body.userBankName;
      const userName = req.body.userName;
      const userStk = req.body.userStk;
      const userIfsc = req.body.userIfsc;
      ...
      insertQuery = `
        INSERT INTO user_bank (phone, tp, name_bank, name_user, stk, chi_nhanh,time)
        VALUES (?, ?, ?, ?, ?, ?,?)
      `;
      insertValues = [
        userPhone,
    ...
    } else if (userType === "USDT_ADDRESS") {
      const crypto_network = req.body.crypto_network;
      const crypto_wallet = req.body.crypto_wallet;
      ...
      insertQuery = `
        INSERT INTO user_bank (phone, tp, name_bank, stk, name_user, chi_nhanh,time)
        VALUES (?, ?, ?, ?, NULL, 0,?)
      `;
      insertValues = [
        userPhone,
    ...
    if (updateResult.affectedRows === 0) {
      //console.log("⚠️ No update, inserting instead...");
