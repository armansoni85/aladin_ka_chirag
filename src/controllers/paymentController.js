...
const addManualUPIPaymentRequest = async (req, res) => {
  try {
    ...
    const [isUsedUtr] = await connection.query(
      "SELECT * FROM recharge WHERE utr = ? ORDER BY id DESC",
      [utr],
    );
    if (isUsedUtr.length) {
      return res.status(400).json({
...
const initiateManualUSDTPayment = async (req, res) => {
  ...
  const [bank_recharge_momo] = await connection.query(
    "SELECT * FROM bank_recharge WHERE type = 'momo' ORDER BY id DESC",
  );
...
const addManualUSDTPaymentRequest = async (req, res) => {
  try {
    ...
    let money = money_usdt * parseInt(process.env.USDT_INR_EXCHANGE_RATE);
    let utr = data.utr;
    let networkType = data.networkType;
    let upi_id = data.upi_id;
    const minimumMoneyAllowed = parseInt(process.env.MINIMUM_MONEY_USDT);

    let [alreadyExist] = await connection.query(
...
const browseRechargeRecord = async (req, res) => {
  try {
    ...
    const [recharge] = await connection.query(
      `SELECT * FROM recharge WHERE status = 0 AND (type = '${PaymentMethodsMap.UPI_MANUAL}' OR type = '${PaymentMethodsMap.USDT_MANUAL}') ORDER BY id DESC`,
      [],
    );
...
const rechargeTable = {
  ...
  create: async (newRecharge) => {
    ...
    await connection.query(
      `INSERT INTO recharge SET id_order = ?, transaction_id = ?, phone = ?, money = ?, type = ?, status = ?, today = ?, url = ?, time = ?, time_remaining_bet = ?, utr = ?,network_type = ?, deposite_address = ?`,
      [
        newRecharge.orderId,
